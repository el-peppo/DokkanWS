...
router.get('/status', async (_req, res) => {
    try {
        ...
        await logger.error('Error getting scrape status:', {}, error);
        res.status(500).json({
            success: false,
...
router.get('/progress', async (_req, res) => {
    try {
        ...
        await logger.error('Error getting scrape progress:', {}, error);
        res.status(500).json({
            success: false,
...
router.post('/trigger', async (_req, res) => {
    try {
        ...
        await logger.error('Error triggering scrape:', {}, error);
        res.status(500).json({
            success: false,
