...
import { TransformationExtractor } from './extractors/transformation-extractor.js';
export class CharacterExtractor {
     ...
     */
    static async extractQuote(source) {
    ...
    static async extractTransformations(source) {
        try {
            // Use the specialized transformation extractor
            const transformations = await TransformationExtractor.extractTransformations(source);
            if (transformations.length > 0) {
                await logger.info(`Found ${transformations.length} transformation(s)`);
                // Log transformation details for debugging
                for (let i = 0; i < transformations.length; i++) {
                    const transform = transformations[i];
                    await logger.debug(`Transformation ${i + 1}: ${transform.name} - ${transform.condition}`);
                }
            }
