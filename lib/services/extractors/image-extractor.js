...
export class ImageExtractor {
    ...
    static async extractImageURL(source) {
        try {
            ...
            const fallbackSrc = await DOMParserAdapter.extractAttribute(source, '.mw-parser-output img', 'src', 'Error');
            return fallbackSrc;
        }
        catch (error) {
    ...
    static async extractQuote(source) {
        try {
            ...
            const quoteText = await DOMParserAdapter.extractText(source, 'h3:contains("Quote") + p, .quote-section, blockquote', 'Error');
     ...
     */
    static async verifyImageURL(imageUrl) {
        try {
