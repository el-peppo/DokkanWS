...
export class JsonImporter {
    ...
    async importFromFile(filePath: string): Promise<void> {
        try {
            ...
            const characters: Character[] = Array.isArray(data) ? data : data.characters;
    ...
    private async characterExists(characterId: string): Promise<boolean> {
        const result = await this.db.query(
            'SELECT COUNT(*) as count FROM characters WHERE id = ?',
            [characterId]
        );
    ...
    private async insertCharacter(character: Character): Promise<void> {
        const sql = `
            INSERT INTO characters (
                ...
                image_url, leader_skill, eza_leader_skill, super_attack, eza_super_attack,
                ultra_super_attack, eza_ultra_super_attack, passive, eza_passive,
                active_skill, active_skill_condition, eza_active_skill, eza_active_skill_condition,
        ...
        const values = [
            ...
            character.activeSkillCondition,
            character.ezaActiveSkill,
            character.ezaActiveSkillCondition,
            character.transformationCondition,
    ...
    private async insertCharacterLinks(characterId: string, links: string[]): Promise<void> {
        ...
        for (const link of links) {
            if (link !== 'Error') {
                await this.db.query(
                    'INSERT IGNORE INTO character_links (character_id, link_name) VALUES (?, ?)',
                    [characterId, link]
                );
            }
        }
    ...
    private async insertCharacterCategories(characterId: string, categories: string[]): Promise<void> {
        ...
        for (const category of categories) {
            if (category !== 'Error') {
                await this.db.query(
                    'INSERT IGNORE INTO character_categories (character_id, category_name) VALUES (?, ?)',
    ...
    private async insertCharacterKiMeter(characterId: string, kiMeter: string[]): Promise<void> {
        ...
        for (let i = 0; i < kiMeter.length; i++) {
            if (kiMeter[i] !== 'Error') {
                await this.db.query(
                    'INSERT IGNORE INTO character_ki_meter (character_id, ki_value, position) VALUES (?, ?, ?)',
                    [characterId, kiMeter[i], i]
                );
            }
        }
    }

    private async insertTransformations(characterId: string, transformations: any[]): Promise<void> {
        for (let i = 0; i < transformations.length; i++) {
            ...
            const sql = `
                INSERT INTO character_transformations (
                    ...
                    transformed_eza_super_attack, transformed_ultra_super_attack,
                    transformed_eza_ultra_super_attack, transformed_passive,
                    transformed_eza_passive, transformed_active_skill,
            ...
            const values = [
                ...
                transformation.transformedEZASuperAttack,
                transformation.transformedUltraSuperAttack,
                transformation.transformedEZAUltraSuperAttack,
                transformation.transformedPassive !== 'Error' ? transformation.transformedPassive : null,
                transformation.transformedEZAPassive,
                transformation.transformedActiveSkill,
                transformation.transformedActiveSkillCondition,
            ...
            if (transformation.transformedLinks && Array.isArray(transformation.transformedLinks)) {
                for (const link of transformation.transformedLinks) {
                    if (link !== 'Error') {
                        await this.db.query(
                            'INSERT IGNORE INTO transformation_links (transformation_id, link_name) VALUES (?, ?)',
                            [transformationId, link]
                        );
                    }
                }
            }
