...
let transformCharacterDocument: Document;
let transformCharacterData: Character | null;
let multiTransformEZACharacterDocument: Document;
let multiTransformEZACharacterData: Character | null;
let EZAActiveCharacterDocument: Document;
let EZAActiveCharacterData: Character | null;
let standardCharacterDocument: Document;
let standardCharacterData: Character | null;
let transformEZALRCharacterDocument: Document;
...
async function fetchFromWeb(url: string): Promise<Document | null> {
  const html = await httpClient.fetchWithRetry(url);
  if (!html) return null;
  return DOMParser.parseHTML(html);
}

before(async function () {
  this.timeout(15000);
  
  transformCharacterDocument = await fetchFromWeb('https://dbz-dokkanbattle.fandom.com/wiki/Divine_Warriors_with_Infinite_Power_Super_Saiyan_God_Goku_%26_Super_Saiyan_God_Vegeta') as Document;
  transformCharacterData = CharacterExtractor.extractCharacterData(transformCharacterDocument);

  multiTransformEZACharacterDocument = await fetchFromWeb('https://dbz-dokkanbattle.fandom.com/wiki/Boiling_Power_Super_Saiyan_Goku#Super_Saiyan') as Document;
  multiTransformEZACharacterData = CharacterExtractor.extractCharacterData(multiTransformEZACharacterDocument);

  EZAActiveCharacterDocument = await fetchFromWeb('https://dbz-dokkanbattle.fandom.com/wiki/Wings_Spread_Out_to_the_Cosmos_Pan_(GT)_(Honey)') as Document;
  EZAActiveCharacterData = CharacterExtractor.extractCharacterData(EZAActiveCharacterDocument);

  standardCharacterDocument = await fetchFromWeb('https://dbz-dokkanbattle.fandom.com/wiki/A_Promise_Made_to_Kakarot_Super_Saiyan_2_Vegeta_(Angel)') as Document;
  standardCharacterData = CharacterExtractor.extractCharacterData(standardCharacterDocument);

  transformEZALRCharacterDocument = await fetchFromWeb('https://dbz-dokkanbattle.fandom.com/wiki/Fused_Fighting_Force_Super_Saiyan_Goku_(Angel)_%26_Super_Saiyan_Vegeta_(Angel)') as Document;
  transformEZALRCharacterData = CharacterExtractor.extractCharacterData(transformEZALRCharacterDocument);
