...
export class PlaywrightParser {
    ...
    private config: PlaywrightParserConfig;
    ...
    async initialize(): Promise<void> {
        try {
            ...
            await logger.error('Failed to initialize Playwright:', {}, error as Error);
            throw error;
        }
    }
    ...
    async navigateToPage(url: string): Promise<Page> {
        ...
        try {
            ...
            await logger.error(`Failed to navigate to ${url}:`, { url }, error as Error);
     ...
     */
    async parseHTML(html: string): Promise<Page> {
        const context = this.getNextContext();
        const page = await context.newPage();
     ...
     */
    async extractCharacterLinks(page: Page, baseUrl: string): Promise<string[]> {
        try {
     ...
     */
    async extractPaginationUrls(page: Page, currentUrl: string): Promise<string[]> {
        try {
     ...
     */
    async querySelector(page: Page, selector: string, timeout: number = 5000): Promise<any> {
        try {
     ...
     */
    async querySelectorAll(page: Page, selector: string): Promise<any[]> {
        try {
     ...
     */
    async extractText(page: Page, selector: string, fallback: string = 'Error'): Promise<string> {
        try {
     ...
     */
    async extractTextWithKiSpheres(page: Page, selector: string, fallback: string = 'Error'): Promise<string> {
        try {
            const element = await page.$(selector);
            if (element) {
     ...
     */
    private replaceKiSphereIcons(html: string): string {
        return html
     ...
     */
    async extractAttribute(page: Page, selector: string, attribute: string, fallback: string = 'Error'): Promise<string> {
        try {
            const element = await page.$(selector);
            if (element) {
    ...
    async takeScreenshot(page: Page, filename: string): Promise<void> {
        try {
            ...
            await logger.error(`Failed to take screenshot ${filename}:`, {}, error as Error);
        }
    }
     ...
     */
    async getStats(): Promise<{ contextsCount: number; pagesCount: number }> {
    ...
    async destroy(): Promise<void> {
        try {
            ...
            await logger.error('Error during Playwright cleanup:', {}, error as Error);
        }
    }
}