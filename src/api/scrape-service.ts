...
export class ScrapeService extends EventEmitter {
    ...
    constructor() {
        ...
        this.scraper = null as any; // Will be initialized in connect()
    }
    ...
    async getStatus(): Promise<ScrapeStatus> {
        try {
            ...
            await logger.error('Failed to get scrape status:', {}, error as Error);
            throw error;
        }
    }
    ...
    private setupProgressTracking(): void {
        ...
        const updateProgress = (category: string, processed: number, total: number) => {
        ...
        const simulateProgress = () => {
            ...
            const interval = setInterval(() => {
                ...
                if (processed % 200 === 0) categoryIndex++;
                
                updateProgress(
                    categories[Math.min(categoryIndex, categories.length - 1)],
    ...
    private async importToDatabase(result: any): Promise<void> {
